          ...
          children: [
            ...
            ElevatedButton(
                onPressed:() async{
                  name = userTC.text;
                  pass = passTC.text;
                  List<dynamic> data = await MongoDataBases.registration(name, pass);
                  if(data[1])
                    {
                ...
                child: const Text("Register",style: TextStyle(fontSize: 20.0,color: Colors.black),)
            ),
            TextButton(onPressed:(){Navigator.pushReplacementNamed(context, '/login');}, child: const Text("already have an account | SignIn",style: TextStyle(color: Colors.indigoAccent),)),
          ],
        ),
      ),
